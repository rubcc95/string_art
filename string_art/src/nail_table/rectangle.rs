...
unsafe impl<N: nails::Handle> NailTable for Rectangle<N> {
    ...
    unsafe fn segment_idx(
        ...
        match nail_idx {
            Id::TopLeft => {
                let start = self.top_left_idx(link) + N::Links::LEN * offset + other_link.into();
                if let Some(rem) = offset.checked_sub(self.height) {
                    if let Some(rem) = rem.checked_sub(1) {
                        (Id::Bot(rem), self.segments.get_unchecked_mut(start))
                    } else {
                        (Id::BotRight, self.segments.get_unchecked_mut(start))
                    }
                } else {
                    (Id::Right(offset), self.segments.get_unchecked_mut(start))
                }
            }
            Id::Top(idx) => {
                let start = self.top_idx(idx, link) + N::Links::LEN * offset + other_link.into();
                if let Some(rem) = offset.checked_sub(self.height) {
                    if let Some(rem) = rem.checked_sub(1) {
                        if let Some(rem) = rem.checked_sub(self.width) {
                            if let Some(rem) = rem.checked_sub(1) {
                                (Id::Left(rem), self.segments.get_unchecked_mut(start))
                            } else {
                                (Id::BotLeft, self.segments.get_unchecked_mut(start))
                            }
                        } else {
                            (Id::Bot(rem), self.segments.get_unchecked_mut(start))
                        }
            ...
            Id::TopRight => {
                let start = self.top_right_idx(link);
                if let Some(rem) = offset.checked_sub(self.height) {
                    if let Some(rem) = rem.checked_sub(1) {
                        (
                            Id::Bot(rem),
                            self.segments.get_unchecked_mut(
                                start + offset * N::Links::LEN + other_link.into(),
                            ),
                        )
                    } else {
                        (
                    ...
                    (
                        Id::Bot(offset),
                        self.segments
                            .get_unchecked_mut(start + offset * N::Links::LEN + other_link.into()),
                    )
                }
            }
            Id::Right(idx) => {
                if let Some(rem) = offset.checked_sub(self.width) {
                    if let Some(rem) = rem.checked_sub(1) {
                        if let Some(rem) = rem.checked_sub(self.height) {
                            if let Some(rem) = rem.checked_sub(1) {
                                let start = self.top_idx(rem, other_link);
                                (
                                    Id::Top(rem),
                                    self.segments.get_unchecked_mut(
                                        start + N::Links::LEN * idx + link.into(),
                                    ),
                                )
                            } else {
                                let start = self.top_left_idx(other_link);
                                (
                                    Id::TopLeft,
                                    self.segments.get_unchecked_mut(
                                        start + N::Links::LEN * idx + link.into(),
                            ...
                            let start = self.right_idx(idx, link);
                            (
                                Id::Left(rem),
                                self.segments.get_unchecked_mut(
                                    start + N::Links::LEN * offset + other_link.into(),
                                ),
                            )
                        }
                    } else {
                    ...
                    (
                        Id::Bot(offset),
                        self.segments
                            .get_unchecked_mut(start + N::Links::LEN * offset + other_link.into()),
                    )
                }
            }
            Id::BotRight => {
                if let Some(rem) = offset.checked_sub(self.height) {
                    if let Some(rem) = rem.checked_sub(1) {
                        ...
                        (
                            Id::Top(rem),
                            self.segments.get_unchecked_mut(
                                start + N::Links::LEN * self.height + link.into(),
                            ),
                        )
                    } else {
                        let start = self.top_left_idx(other_link);
                    ...
                    let start = self.bot_right_idx(link);
                    (
                        Id::Left(offset),
                        self.segments
                            .get_unchecked_mut(start + N::Links::LEN * offset + other_link.into()),
            ...
            Id::Bot(idx) => {
                if let Some(rem) = offset.checked_sub(self.height) {
                    if let Some(rem) = rem.checked_sub(1) {
                        if let Some(rem) = rem.checked_sub(self.width) {
                            if let Some(rem) = rem.checked_sub(1) {
                                let start = self.right_idx(rem, other_link);
                                (
                                    Id::Right(rem),
                                    self.segments.get_unchecked_mut(
                                        start + N::Links::LEN * idx + link.into(),
                                ...
                                let start = self.top_right_idx(other_link);
                                (
                                    Id::TopRight,
                                    self.segments.get_unchecked_mut(
                                        start + idx * N::Links::LEN + link.into(),
                                    ),
                                )
                            }
                        } else {
                            ...
                            (
                                Id::Top(rem),
                                self.segments.get_unchecked_mut(
                                    start + N::Links::LEN * (self.height + 1 + idx) + link.into(),
                                ),
                            )
                        }
                    } else {
                    ...
                    let start = self.bot_idx(idx, link);
                    (
                        Id::Left(offset),
                        self.segments
                            .get_unchecked_mut(start + N::Links::LEN * offset + other_link.into()),
                ...
                if let Some(rem) = offset.checked_sub(self.width) {
                    if let Some(rem) = rem.checked_sub(1) {
                        ...
                        (
                            Id::Right(rem),
                            self.segments.get_unchecked_mut(
                                start + N::Links::LEN * self.width + link.into(),
                            ),
                        )
                    } else {
                        let start = self.top_right_idx(other_link);
                        (
                            Id::TopRight,
                            self.segments.get_unchecked_mut(
                                start + self.width * N::Links::LEN + link.into(),
                            ),
                        )
                    }
                } else {
                    let start = self.top_idx(offset, other_link);
                    (
                        Id::Top(offset),
                        self.segments.get_unchecked_mut(
                            start + N::Links::LEN * (self.height + self.width + 1) + link.into(),
                        ),
                    )
                }
            }
            Id::Left(idx) => {
                if let Some(rem) = offset.checked_sub(self.width) {
                    if let Some(rem) = rem.checked_sub(1) {
                        if let Some(rem) = rem.checked_sub(self.height) {
                            if let Some(rem) = rem.checked_sub(1) {
                                let start = self.bot_idx(rem, other_link);
                                (
                                    Id::Bot(rem),
                                    self.segments.get_unchecked_mut(
                                        start + N::Links::LEN * idx + link.into(),
                                ...
                                let start = self.bot_right_idx(other_link);
                                (
                                    Id::BotRight,
                                    self.segments.get_unchecked_mut(
                                        start + N::Links::LEN * idx + link.into(),
                            ...
                            (
                                Id::Right(rem),
                                self.segments.get_unchecked_mut(
                                    start + N::Links::LEN * (self.height + idx) + other_link.into(),
                                ),
                            )
                        }
                    } else {
                        ...
                        (
                            Id::TopRight,
                            self.segments.get_unchecked_mut(
                                start + (self.width + 1 + idx) * N::Links::LEN + link.into(),
                            ),
                        )
                    }
                } else {
                    ...
                    (
                        ...
                        self.segments.get_unchecked_mut(
                            start
                                + N::Links::LEN * (self.width + self.height + 2 + idx)
                                + link.into(),
                        ),
                    )
                }
